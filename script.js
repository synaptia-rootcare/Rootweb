document.addEventListener('DOMContentLoaded', () => {
  ...
  links.forEach(a=>{
    a.addEventListener('click', e=>{ e.preventDefault(); smoothTo(a.getAttribute('href')); });
  });
  ...
  document.querySelectorAll('.tab').forEach(t=>t.addEventListener('click',()=>{
    ...
    ['auto','remote','quick'].forEach(id=>{
      const pane=document.getElementById('pane-'+id);
      if(pane) pane.style.display = (t.dataset.pane===id)?'block':'none';
    });
  }));
  ...
  let idx=0;
  setInterval(()=>{ steps.forEach(s=>s.classList.remove('active')); if(steps.length){ steps[idx%steps.length].classList.add('active'); idx++; }}, 2200);

  // Ecosystem: click -> toggle image/description
  document.getElementById('eco-pop')?.remove(); // kill legacy popup if present
